...
for i in range(len(poster_ozellikleri)):
    print(i, time.time())
    benzerlik_skorlari = cosine_similarity(
        poster_ozellikleri[i].reshape(1, -1), poster_ozellikleri)
    benzerlik_skorlari = benzerlik_skorlari[0]
    en_yakin_posterler = np.argsort(benzerlik_skorlari)[-26:-1]
    new_list.clear()
    for j in reversed(en_yakin_posterler):
        new_list.append([poster_dosyalari[j][:-4],
                        "{:.2f}".format(benzerlik_skorlari[j])])
    poster_similarities[poster_dosyalari[i][:-4]] = new_list

with open('poster_similarities.json', 'w', encoding='utf-8') as f:
    json.dump(poster_similarities, f, ensure_ascii=False,
